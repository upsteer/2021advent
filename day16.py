import sys

file = ""
if str(sys.argv[len(sys.argv)-1]) == 'test':
	file = "tests/"
data_file = file + "day16.txt"

with open(data_file, 'r') as f:
    input = f.read()

sum=0
def get_literal_value(binary_input):
	finale = ""
	ver = int(binary_input[0:3], 2)
	type_id = int(binary_input[3:6], 2)
	print('binn', binary_input)
	print("header ver", ver)
	print("header type id", type_id)
	global sum
	# sum+=ver
	# print(int(binary_input[0:3], 2))
	end = 0
	for num in range(6,len(binary_input), 5):
		finale+=binary_input[num+1:num+5]
		end = num
		if binary_input[num] == "0":
			end+=5
			break
	print(f"ended {end}")
	return end

# hex_inp = "38006F45291200"
# hex_inp = "EE00D40C823060"
# hex_inp = "8A004A801A8002F478"
hex_inp = "620080001611562C8802118E34"
hex_inp = "C0015000016115A2E0802F182340"
hex_inp = "A0016C880162017C3686B18A3D4780"
hex_inp = ""
init_bin_inp = bin(int(hex_inp, 16))[2:]
div_four = len(init_bin_inp) % 4
if div_four != 0:
	for _ in range(4-div_four):
		init_bin_inp = "0" + init_bin_inp
print("after padding", init_bin_inp)
def get_packs(bin_inp):
	global sum
	if not bin_inp:
		return ""
	pack_version = bin_inp[0:3]
	type_id = bin_inp[3:6]
	if not (pack_version):
		return ""
	if not type_id:
		return ""
	pack_version = int(pack_version, 2)
	print('pv', pack_version)
	type_id = int(type_id, 2)
	print(f"typeid {type_id} and pack {pack_version}")
	length_type_id = bin_inp[6]
	sum+=pack_version
	skipped = 6
	if type_id == 4:
		print("literal ret")
		skipped+=get_literal_value(bin_inp)
		return get_packs(bin_inp[skipped:])
	else:
		skipped += 1
		if length_type_id == "0":
			print("zero len type id")
			#Total no. of sub-packets in this packet
			till = int(bin_inp[7:22], 2)
			consumed = 0
			index = 22
			skipped+=15
			while consumed<till:
				value = get_packs(bin_inp[index:])
				consumed+=len(value)
				if not value:
					return ""
			skipped+=till
		else:
			print("one len type id")
			#Length of bits to read from this packet
			num_of_sub = int(bin_inp[7:18], 2)
			index = 0
			value_1 = 18
			for _ in range(num_of_sub):
				value_1 += len(get_packs(bin_inp[value_1:]))
				if not value_1:
					return ""
			skipped+=value_1+1
		print('remaining', bin_inp[skipped+1:])
	if skipped > len(bin_inp):
		return get_packs(bin_inp[skipped+1:])
	return ""

get_packs(init_bin_inp)
print(sum)